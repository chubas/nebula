...
Minion = Actor.extend({
    ...
    _bindEvents : function () {
      ...
      minion.viewRadius.bind('spotted', function (ev) {
        if (ev.who.name === "Minion" && minion.ancestor !== ev.who.ancestor) {
          if (minion.state !== 'idle') {
            minion._follow(ev.who, ev.priority);
          }
        }

        // The player is being followed.
        if (ev.who.name === "View Radius" &&
            ev.who.ancestor.name === "Player" &&
            ev.who.ancestor.state !== "aiming" &&
            ev.who.ancestor.ancestor === minion.ancestor) {
          if (minion.state !== 'idle') {
            minion._follow(ev.who, ev.priority);
            minion.hype = true;
          }
      ...
      minion.listenRadius.bind('spotted', function (ev) {
        if (ev.who.name === "View Radius" && ev.who.ancestor.ancestor === minion.ancestor && ev.who.ancestor.state === "following") {
          if (minion.state !== 'idle') {
